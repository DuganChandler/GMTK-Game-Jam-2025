...
public class BallBro : MonoBehaviour, ILightReciever, ILightReflector
{
    ...
    private List<BeamData> currentlyRecievingBeams;
    ...
    private void Awake()
    {
        ...
        spawnedLines = new LineRenderer[1];

        if (isSource)
        {
            IsLit = true;
            currentlyReflectedBeams.Add(new(level1Color, 1, lineSpawnPoint.position, transform.forward, this, this, null, null));
        }
    }
    ...
    public void OnBeamRecieved(BeamData data)
    {
        ...
        BeamData beamToShoot = new(GetLightColorFromPower(), reflectedBeamLevel, lineSpawnPoint.position, lineSpawnPoint.forward, this, this, null, null);
        currentlyReflectedBeams.Add(beamToShoot);
    ...
    public void ReflectBeams()
    {
        for (int i = 0; i < currentlyReflectedBeams.Count; i++)
        {
            // Raycast
            bool hitSomething;
            //hitSomething = Physics.Raycast(currentlyReflectedBeams[i].Origin, transform.forward, out RaycastHit castHit, lineLength, lineHitLayers);
            hitSomething = Physics.SphereCast(lineSpawnPoint.position, 0.05f, transform.forward, out RaycastHit castHit, lineLength, lineHitLayers);
            ...
            {
                lightBeamEndPos = Vector3.forward * lineLength;
            }
            lightBeamEndPos.y = 0;
    ...
    private void DeactivateLight()
    {
        ...
        {
            if (spawnedLines[i] != null) Destroy(spawnedLines[i].gameObject);
        }
    }
    #endregion
